...
std::ostream& write_VTK(std::ostream& out, const Delaunay& m_dela, const std::vector<std::vector<int> >& simplex_id, Vtk_exported_cells opts = CELLS|EDGES|VERTICES, std::vector<std::vector<int> >* flags = NULL, std::vector<std::vector<Tflag2> >* flags2 = NULL) {
    ...
    if (opts & EDGES) {
        ...
        for (typename Delaunay::Edge edge : m_dela.finite_edges()) {
            ...
            dims.push_back(1);
            ids.push_back(simplex_id.at(1).at(cpt++));
            out << std::endl;
        }
    }
    // Export triangles
    if (opts & FACETS) {
        ...
        for (typename Delaunay::Facet facet : m_dela.finite_facets()) {
            ...
            dims.push_back(2);
            ids.push_back(simplex_id.at(2).at(cpt++));
            out << std::endl;
        }
    }
    // Export cells
    if (opts & CELLS) {
        ...
        for (typename Delaunay::Cell_handle cell : m_dela.finite_cell_handles()) {
            ...
            dims.push_back(3);
            ids.push_back(simplex_id.at(3).at(cpt++));
            out << std::endl;
        }
    }
    out << std::endl;
...
template <typename Tflag2>
void write_VTK(const Delaunay& m_dela, std::string filename, const std::vector<std::vector<int> >& simplex_id, Vtk_exported_cells opts = CELLS|EDGES|VERTICES, std::vector<std::vector<int> >* flags = NULL, std::vector<std::vector<Tflag2> >* flags2 = NULL) {
    ...
    write_VTK(out, m_dela, simplex_id, opts, flags, flags2);
    out.close();
}

// Added by YS, because calling write_VTK without flags resulted in error: no match.
//(couldn’t deduce template parameter Tflag2)
void write_VTK(const Delaunay& m_dela, std::string filename, const std::vector<std::vector<int> >& simplex_id, Vtk_exported_cells opts = CELLS|EDGES|VERTICES) {
    std::ofstream out ( filename, std::ios::out | std::ios::trunc);
    ...
    write_VTK(out, m_dela, simplex_id, opts, NULL, flags_null);
    out.close();
}
